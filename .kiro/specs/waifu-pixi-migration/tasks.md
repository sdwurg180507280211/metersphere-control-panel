...
- [ ] 0. 准备阶段
  ...
  - [ ] 0.4 调研并确认 pixi-live2d-display 与当前项目依赖兼容矩阵
    - 检查 PixiJS 主版本、React 环境、构建链路与 `pixi-live2d-display` 的兼容性
    - 明确最终采用的 PixiJS 组合，以及是否需要完整 Pixi 构建或手动注册 interaction 插件
    - 输出依赖版本建议与 PoC 结论
    - 明确模型资源目录、Cubism runtime 来源与 CORS 策略
    - 验证新引擎不需要依赖 `window.Image` 全局 patch
    - _Requirements: 18.1, 19.1, 20.1, 21.1-21.4_

  - [ ] 0.5 建立最小 PoC：在 React 页面中加载一个 Live2DModel
    - 使用 `Live2DModel.from(...)` 完成最小模型加载与渲染
    - 验证 StrictMode 下初始化与销毁是否稳定
    - 验证快速切模、过期异步结果和 destroy 中断场景下不会残留模型实例
    - 验证模型 JSON、贴图、motion、expression 与 runtime 的实际加载链路
    - 记录开发环境与生产环境下的资源路径解析结果
    - _Requirements: 18.2, 20.1, 16.1-16.5, 21.2-21.5_

  - [ ] 0.6 验证库内 motion / expression / hit test / focus 能力边界
    - 针对至少一个目标模型验证库 API 是否满足迁移需求
    - 验证 `tap()` / `hitTest()` 的命中语义与业务点击反馈编排边界
    - 验证 drag 需要由业务层自行实现，focus / hit test 仅作为基础能力复用
    - 验证 motion / expression / ParamController overlay 的共存时序，以及 lip-sync 需由业务层主实现
    - 记录能力限制与替代方案
    - _Requirements: 18.3, 20.2, 20.3, 20.4, 8.6_

  - [ ] 0.7 Checkpoint - 前置验证完成
    - 确认依赖兼容矩阵、PoC 和库能力验证清单已完成
    - 如存在关键能力缺口，更新 design.md
    - 若兼容矩阵、PoC 或能力边界验证未通过，则暂停后续大规模实现
