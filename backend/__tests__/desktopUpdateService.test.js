...
function loadTransport(fetchMock, electron = false, timers = {}) {
  ...
  vm.runInNewContext(
    `${fs.readFileSync(filename, 'utf8')}\nmodule.exports = { requestText, downloadFile, checkForUpdate, createHelperScript };`,
    sandbox
  );
  return sandbox.module.exports;
}

function writeStubBin(bin, commands) {
...
describe('Desktop updater network transport', () => {
  ...
  test.each([false, true])('installer helper replaces or rolls back a temporary app (launch fails: %s)', launchFails => {
    ...
    try {
      ...
      expect(fs.readFileSync(path.join(directory, 'update-helper.log'), 'utf8'))
        .toContain(launchFails ? '恢复旧版本' : 'update completed');
    } finally {
  ...
  test('rejects an HTTPS downgrade and excessive redirects', async () => {
    const downgrade = jest.fn(async () => new Response(null, {
      status: 302,
      headers: { location: 'http://github.com/test' }
    }));
  ...
  test('rejects HTTP errors and oversized metadata', async () => {
    await expect(loadTransport(async () => new Response('', { status: 503 })).requestText('https://github.com/test'))
      .rejects.toThrow('503');
    await expect(loadTransport(async () => new Response('x'.repeat(2 * 1024 * 1024 + 1))).requestText('https://github.com/test'))
      .rejects.toThrow('元数据过大');
  });
  ...
  test('failed download streams clean up partial files', async () => {
    ...
    try {
      ...
      await expect(loadTransport(async () => new Response(body)).downloadFile('https://github.com/test', destination))
  ...
  test('streams downloads, hashes bytes and rejects untrusted redirect hosts', async () => {
    ...
    try {
      ...
      const redirect = jest.fn(async () => new Response(null, {
        status: 302,
        headers: { location: 'https://example.com/test' }
      }));
      await expect(loadTransport(redirect).downloadFile('https://github.com/test', destination))
        .rejects.toThrow('不允许的下载主机');
...
describe('Delta updates', () => {
  ...
  const RELEASES_RESPONSE = () => new Response(JSON.stringify([{
    tag_name: 'desktop-v2.0.8',
    draft: false,
    prerelease: false,
    assets: [{
      name: 'latest.json',
      browser_download_url: `https://github.com/${REPOSITORY}/releases/download/desktop-v2.0.8/latest.json`
    }]
  }]));
    ...
    assets: [
      {
        ...
        bytes: 130000000,
        sha256: 'b'.repeat(64),
        url: `https://github.com/${REPOSITORY}/releases/download/desktop-v2.0.8/full.zip`
      },
      {
        name: 'full-arm64.zip',
        arch: 'arm64',
        ...
        bytes: 125000000,
        sha256: 'c'.repeat(64),
        url: `https://github.com/${REPOSITORY}/releases/download/desktop-v2.0.8/full-arm64.zip`
      }
    ],
    deltas: [{
      name: 'delta.zip',
      ...
      bytes: 4800000,
      sha256: 'd'.repeat(64),
      url: `https://github.com/${REPOSITORY}/releases/download/desktop-v2.0.8/delta.zip`,
  ...
  test('selects the delta asset when the installed Electron version matches', async () => {
    const result = await loadTransport(metadataFetch(), true).checkForUpdate({ currentVersion: '2.0.7', arch: 'x64' });
    ...
    expect(Object.keys(result.asset).sort()).toEqual(['arch', 'bytes', 'electronVersion', 'name', 'sha256', 'updateMode', 'url'].sort());
  });
  ...
  test('delta entries without a name fall back to the full zip', async () => {
    ...
    const result = await loadTransport(metadataFetch(metadata), true).checkForUpdate({ currentVersion: '2.0.7', arch: 'x64' });
  ...
  test('plain Node runtime（开发模式）never selects a delta', async () => {
    const result = await loadTransport(metadataFetch()).checkForUpdate({ currentVersion: '2.0.7', arch: 'x64' });
    expect(result.asset).toMatchObject({ updateMode: 'full' });
  });

  test('installer script contains the delta branch without legacy asset restoration', () => {
    const script = loadTransport(jest.fn()).createHelperScript();
    expect(script).toContain('MODE="${5:-full}"');
    expect(script).toContain('cp -cR "$TARGET_APP" "$NEW_APP"');
    expect(script).not.toContain('${6:-');
    const checked = spawnSync('/bin/bash', ['-n'], { input: script, encoding: 'utf8' });
    expect(checked.status).toBe(0);
  });
  ...
  test('delta helper replaces the complete app layer and does not restore removed legacy assets', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(targetApp, 'frontend', 'dist', 'legacy-assets', 'old-model.txt'), 'old asset');
      ...
      fs.writeFileSync(path.join(stagedApp, 'package.json'), '{"version":"2.0.8"}');
      ...
      expect(fs.existsSync(path.join(newAppDir, 'frontend', 'dist', 'legacy-assets'))).toBe(false);
  ...
  test('delta helper rolls back to the previous bundle when launch fails', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(targetApp, 'package.json'), '{"version":"2.0.7"}');
      fs.writeFileSync(path.join(targetApp, 'legacy-assets', 'old.txt'), 'old asset');

      fs.mkdirSync(path.join(staged, 'Contents', 'Resources', 'app'), { recursive: true });
      fs.writeFileSync(path.join(staged, 'Contents', 'Info.plist'), 'new');
      fs.writeFileSync(path.join(staged, 'Contents', 'Resources', 'app', 'package.json'), '{"version":"2.0.8"}');
      ...
      expect(fs.existsSync(path.join(target, 'Contents', 'Resources', 'app', 'legacy-assets', 'old.txt'))).toBe(true);
      expect(fs.existsSync(`${target}.previous`)).toBe(false);
      expect(fs.existsSync(`${target}.new`)).toBe(false);
