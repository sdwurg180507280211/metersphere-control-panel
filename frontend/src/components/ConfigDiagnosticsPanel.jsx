function ConfigDiagnosticsPanel({ diagnostics, validation, loading, onRefresh }) {
  ...
  return (
    <section className="config-card config-panel-card">
      ...
      <div className="config-diagnostic-grid">
        ...
        <div className="config-diagnostic-wide">
          <DiagnosticBlock title="端口分布" status>
            <div className="config-port-grid">
              {(diagnostics?.ports || []).map((item) => (
                <div key={`${item.port}-${item.owners?.length || 0}`} className="config-port-card">
...
function DiagnosticBlock({ title, status, children }) {
  return (
    <div className="config-diagnostic-block config-diagnostic-card">
